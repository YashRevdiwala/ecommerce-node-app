...
const createSubCategories = async (req, res) => {
  ...
  try {
    ...
    connection.query(query, [catSlug], (err, rows) => {
      if (err) {
        return res.status(codes.ServerError).json({ error: err.message })
      }
      if (rows.length === 0) {
        ...
        connection.query(query, [name, slug], (err, subCat) => {
          if (err) {
            return res.status(codes.ServerError).json({ error: err.message })
          }
          if (subCat.length > 0) {
            ...
            connection.query(query, [name, slug, catSlug], (err, result) => {
              if (err) {
                return res
                  .status(codes.BadRequest)
...
const getCatAndSubCat = async (req, res) => {
  try {
    ...
    connection.query(catQuery, (err, catRows) => {
      if (err) {
        return res.status(codes.ServerError).json({ error: err.message })
      }
      connection.query(subCatQuery, (err, subCatRows) => {
        if (err) {
          return res.status(codes.ServerError).json({ error: err.message })
        }
